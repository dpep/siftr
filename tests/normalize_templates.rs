...
use siftr::normalize::{Normalizer, SlotKind, slot_value_f64};
...
fn strip_ansi_keeps_only_the_text() {
    ...
    siftr::normalize::strip_ansi(line.as_bytes(), &mut out);
    assert_eq!(out, b"  User Load (0.0ms)  SELECT 1");
    siftr::normalize::strip_ansi(b"cut \x1b[3", &mut out);
    assert_eq!(out, b"cut ");
}
...
fn template_hash_is_pinned() {
    ...
    assert_eq!(
        ...
        siftr::normalize::fnv1a64(b"User Load (<duration>)")
    );
    assert_eq!(
        n.normalize(b"User Load (0.3ms)").template_hash,
