...
use siftr::store::{Finished, NewRun, Retention, RunEnd, RunId, Store};
...
fn opening_gives_up_on_a_migration_lock_another_siftr_holds() {
    ...
    assert!(
        error.downcast_ref::<siftr::store::StoreBusy>().is_some(),
        "{error:#}"
    );
    drop(held);
}
...
fn a_run_still_recording_is_never_pruned_however_idle_but_an_abandoned_one_is() {
    ...
    assert!(
        error.downcast_ref::<siftr::store::Pruned>().is_some(),
        "{error:#}"
    );
