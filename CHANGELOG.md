...
- siftr records what the kernel charged every run: CPU time (user and system), peak memory, and voluntary and involuntary context switches. One `getrusage` of the reaped child, ~0.19µs, no sampler — and no change to how your command runs, what it prints, or the code it exits with. It's a source like the others and on by default; `[sources.rusage] enabled = false` turns it off. A command that exited non-zero or died from a signal is measured too.
- **This is evidence, never a signal.** CPU and memory vary far too much run to run to judge — `docs/findings/signals.md` §1 measured a suite's wall time spanning 10x — so `run.resources` is the one behavior kind no rule reads. It cannot raise a change, whatever the numbers do. Upgrading is silent for the same reason: the first run that has it isn't reported NEW, and a run recorded with the source off isn't reported DISAPPEARED.
- `siftr explain` shows the run's CPU and peak memory beside the median of its baseline runs, so a slowdown that came with far more CPU or far more preemption reads as a loaded machine rather than a code change. In `-j` that's `explain`'s `resources` {`current`, `baseline`}, in whole milliseconds and bytes. `siftr summary` lists the behavior, and `siftr evidence <id>` shows the line the numbers came from.
- Only fields the platform actually fills are reported. macOS leaves `ru_inblock`, `ru_oublock` and `ru_majflt` at zero even for a child that fsync'd 64 MiB, so siftr reports no disk-I/O counters rather than zeros dressed as data. `ru_maxrss` is bytes on macOS and kibibytes on Linux; siftr normalizes it at the syscall, so `max_rss_bytes` means the same thing on both.
- `siftr sources` lists `rusage`. It always applies — every run has a child to measure — and shows an em dash where the other sources name a stream, since it reads no bytes and so never appears in a run's `streams`.
