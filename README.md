...
siftr: reading; each behavior is reported the first time it is seen
      ...
      2  log            I, [<timestamp> #<int>] INFO -- : Completed <int> OK in <duration> (Views: <duration> | ActiveRecord: <duration>)
^C
r5: interrupted by signal 2; kept as evidence, not compared, never a baseline
  in this input, not a comparison:
    all 2 behaviors, 12 events
        6  log            I, [<timestamp> #<int>] INFO -- : Completed <int> OK in <duration> (Views: <du…
        6  log            I, [<timestamp> #<int>] INFO -- : Started GET "/users/<int>" for <ip> at <time…
    most time: 27ms over 6 occurrences — I, [<timestamp> #<int>] INFO -- : Completed <int> OK in <duratio…
next: siftr summary r5
```

**A read's answer is stdout**: the streamed behaviors and the report you see above them — the comparison, the description, the `next:` line — are all data, and pipe cleanly. stderr carries only what siftr says *about the reading*: the `reading;` preamble, the unnamed-pipe note, warnings. This is the opposite split from [`siftr run`](#siftr-run----cmd), where siftr's report goes to **stderr** so the wrapped command owns stdout — there, stdout belongs to someone else; here, there is no one else. Only the **first** sighting of a behavior prints, which is what keeps a tail readable rather than a second copy of the log: 100 templates carry 70% of a real log's lines and 1,000 carry 88% ([docs/findings/log-contexts.md](docs/findings/log-contexts.md)), so after a short warmup siftr is quiet until something genuinely novel arrives. It doesn't go *silent* — the one-off tail keeps trickling, about 4.8% of lines on a whole-system log and 0.3% on a narrow one. Past 20,000 distinct behaviors siftr says so once on stderr and reports no more. Only the template ever streams, never a raw line, so no `SIFTR_REDACT` setting changes what it can disclose.

**Ctrl-C ends the input the way EOF does.** The run is kept, reported and described, and — being partial — never compared and never used as a baseline: its missing tail would read as everything disappearing. siftr then dies of the signal rather than exiting 0, so a shell loop around it stops. An interrupted run doesn't poison what follows; `siftr history` marks it `interrupted (signal 2)`, and the next whole read compares as normal.

**A pipe has no name.** `cat a.log | siftr` and `cat b.log | siftr` are the same file descriptor, so siftr cannot tell them apart: every unnamed pipe in a project shares one context, while `siftr app.log`, which does have a name, gets its own. Whenever an unnamed read has no baseline to compare with, siftr says so on stderr:

```
siftr: r5: stdin has no name, so this compares with other unnamed pipes here rather than with `siftr FILE` of the same log; --context NAME gives it one
```
...
Runs recorded in this project, newest first. `--context NAME` filters (a context with no runs is an error, exit 2), `-n` limits. `incomplete` marks a run that skipped examples its baseline ran, so its changes don't mean what a whole run's do; `interrupted (signal 2)` marks one a Ctrl-C ended, which was never compared and is never a baseline.
...
**Gating on a log's counts? Give it more than two baseline runs.** A test suite's counts are deterministic — the same suite runs the same queries — and on a count that holds still FREQUENCY never fires at all. A **log's** counts vary by nature, and there a young context is noisy in a way that is worth budgeting for: measured over synthetic traffic, FREQUENCY fires on about **a quarter of comparisons at 2 baseline runs**, falling to 7.5% at 3, 3% at 4, 1% at 5 and 0.05% by 10 — regardless of whether the count averages 1 or 100. A signal raised that way stays open for about two further runs before the count wanders back and resolves it — so a gate reading `open_signals` goes red for a couple of runs rather than forever, but it does go red. So on logs, wait for five or so reads before wiring `open_signals` into anything that blocks, and use `siftr ack sN --wrong` when one turns out to be noise — that is the input a precision measurement needs, and the only thing that ends the reminder. Numbers and method: [docs/findings/frequency-small-n.md](docs/findings/frequency-small-n.md).
